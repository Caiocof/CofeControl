<?php


namespace Source\Core;


use Source\Support\Message;

/**
 * Class Session
 * @package Source\Core
 */
class Session
{
    /**
     * Session constructor.
     */
    public function __construct()
    {
        if (!session_id()) {
            session_start();
        }
    }

    /**
     * @param $name
     * @return mixed|null
     */
    public function __get($name)
    {
        if (!empty($_SESSION[$name])) {
            return $_SESSION[$name];
        }
        return null;
    }


    /**
     * @param $name
     * @return bool
     */
    public function __isset($name): bool
    {
        return $this->has($name);
    }


    /**metodo para converte a sessao e um objeto
     * @return object|null
     */
    public function all(): ?object
    {
        return (object)$_SESSION;
    }


    /**metodo para setar uma nova função ele recebera uma chave e um value que pode ser mix
     * @param string $key
     * @param mixed $value
     * @return $this
     */
    public function set(string $key, $value): Session
    {
        $_SESSION[$key] = (is_array($value) ? (object)$value : $value);
        return $this;
    }


    /**metodo para remover a sessao
     * @param string $key
     * @return $this
     */
    public function unset(string $key): Session
    {
        unset($_SESSION[$key]);
        return $this;
    }


    /**metodo para verificar a sessao
     * @param string $key
     * @return bool
     */
    public function has(string $key): bool
    {
        return isset($_SESSION[$key]);
    }


    /**metodo para gerar novo id na sessao
     * @return $this
     */
    public function regeneration(): Session
    {
        session_regenerate_id(true);
        return $this;
    }


    /**metodo de logout, para destruir a sessao
     * @return $this
     */
    public function destroy(): Session
    {
        session_destroy();
        return $this;
    }


    /**
     * @return Message|null
     */
    public function flash(): ?Message
    {
        if ($this->has("flash")) {
            $flash = $this->flash;
            $this->unset("flash");
            return $flash;
        }

        return null;
    }

    /**CRIA UM TOKEN PARA VALIDAR ENVIO DE DADOS NO FORMULARIO EVITANDO ATAQUE DE BOOTS
     * CSRF Token
     */
    public function csrf(): void
    {
        //cria uma criptografica basica para o token
        $_SESSION['csrf_token'] = md5(uniqid(rand(), true));
    }


}